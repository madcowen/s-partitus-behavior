...
plot_setrad_JG_2003 <- 
  ...
  labs(title = "B    2003-2008")

plot_setrad_JG_2008 + plot_setrad_JG_2003 + 
  # plot_annotation(tag_levels = c("A")) +
...
p2 <- ggplot(data = predict_gam(age_jg12, values = list(meanTemp_juv = mean(nat_data_2d$meanTemp_juv_2d))), aes(x = juv_age, y = fit)) + 
...
p5 <- ggplot(data = predict_gam(age_jg12, values = list(meanTemp_juv = mean(sixyrdata_2d$meanTemp_juv_2d))), aes(x = juv_age, y = fit)) + 
...
{p1 + ggtitle("2008-2009")} + {p4 + ggtitle("2003-2008")} + p2 + p5 + p3 + p6 +
  plot_layout(nrow = 3) +
  plot_annotation(tag_levels = c("A"))
